...
class fun:
    ...
    def split_benchmarks(self, split, fold, k_fold=None):
        ...
        if k_fold == None:
            ...
            benchmarks_path = f"/mnt/disk023/lcedillo/Facet-MLP/1028_paramadvisor_data_transfer/{split}_VTML200.45.11.42.40-12-{fold}"
            benchmarks = pd.read_csv(benchmarks_path, header=None, sep=r"[\t]", engine="python", names=["benchmarks", "count"]).drop(["count"], axis=1)
            parameters_benchmarks = pd.read_pickle("/mnt/disk023/lcedillo/Facet-MLP/Data/parameters_benchmarks.pkl")

            return pd.merge(parameters_benchmarks, benchmarks)
            
        if split == "train":
            
            train_bench = f"/mnt/disk023/lcedillo/Facet-MLP/1028_paramadvisor_data_transfer/train_VTML200.45.11.42.40-12-{fold}"
            train_bench = pd.read_csv(train_bench, header=None, sep=r"[\t]", engine="python", names=["benchmarks", "count"]).drop(["count"], axis=1)
        
            test_bench = f"/mnt/disk023/lcedillo/Facet-MLP/1028_paramadvisor_data_transfer/test_VTML200.45.11.42.40-12-{k_fold}"
        ...
        if split == "val":
            ...
            benchmarks_path = f"/mnt/disk023/lcedillo/Facet-MLP/1028_paramadvisor_data_transfer/test_VTML200.45.11.42.40-12-{k_fold}"
            benchmarks = pd.read_csv(benchmarks_path, header=None, sep=r"[\t]", engine="python", names=["benchmarks", "count"]).drop(["count"], axis=1)
            parameters_benchmarks = pd.read_pickle("/mnt/disk023/lcedillo/Facet-MLP/Data/parameters_benchmarks.pkl")
